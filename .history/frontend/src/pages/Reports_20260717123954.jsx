...
export default function Reports() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h1 className="text-3xl font-bold text-gray-900 mb-1">Attendance Report</h1>
      <p className="text-gray-500 mb-6">Overview of all your students</p>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-400">
            ...
            <p className="text-sm mt-1">Take attendance first to see the report</p>
          </div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-50">
              {report.map(row => (
                <tr key={row._id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 font-medium text-gray-900">{row.studentName}</td>
                  <td className="px-6 py-4 text-sm text-gray-500 font-mono">{row.rollNumber}</td>
                  <td className="px-6 py-4 text-center font-semibold text-green-600">{row.present}</td>
                  <td className="px-6 py-4 text-center font-semibold text-red-500">{row.absent}</td>
                  <td className="px-6 py-4 text-center text-gray-600">{row.totalClasses}</td>
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                      ...
                      {row.percentage >= 75 ? 'Present ✅' : row.percentage >= 50 ? 'Low ⚠️' : 'Critical ❌'}
                    </span>
                  </td>
                </tr>
              ))}
      ...
      {report.filter(r => r.percentage < 75).length > 0 && (
        <div className="mt-4 bg-red-50 border border-red-200 rounded-2xl p-5">
          <p className="font-semibold text-red-800 mb-2">⚠️ Students Below 75%</p>
          <div className="flex flex-wrap gap-2">
            {report
              .filter(r => r.percentage < 75)
              .map(r => (
                <span key={r._id} className="bg-white border border-red-200 text-red-700 px-3 py-1 rounded-full text-sm font-medium">
