...
export default function TakeAttendance() {
  ...
  const markAbsentStudents = async (presentIds, subjectVal) => {
    const absentStudents = allStudentsRef.current.filter(s => !presentIds.has(s._id));
    ...
    toast(`📋 ${absentStudents.length} student(s) marked absent`, { duration: 3000 });
  };
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        const [allRes, faceRes] = await Promise.all([
          axios.get('/api/students'),
          axios.get('/api/students/with-descriptors')
        ]);

        setAllStudents(allRes.data.students);
        allStudentsRef.current = allRes.data.students;
        ...
        setStatus(`✅ Ready — ${allRes.data.students.length} students loaded`);
      } catch {
  ...
  const markAttendanceForStudent = async (student, confidence, subjectVal, classRoomVal, semesterVal) => {
    try {
      await axios.post('/api/attendance/mark', {
        studentId: student._id,
        subject:   subjectVal,
      ...
      if (err.response?.status === 409) {
        setMarkedAttendance(prev =>
          prev.find(a => a.id === student._id) ? prev : [...prev, {
            id:         student._id,
            name:       student.name,
            rollNumber: student.rollNumber,
            confidence: Math.round(confidence * 100),
    ...
    try {
      ...
      resized.forEach(detection => {
        ...
        const match = labeledRef.current.length > 0
          ? findBestMatch(detection.descriptor, labeledRef.current) : null;
        ...
        const student = isKnown
          ? studentsRef.current.find(s => s._id === match.label) : null;
        const label = student
  ...
  const startScanning = async () => {
    if (!subjectRef.current.trim()) {
      toast.error('Please enter a subject name first'); return;
    }
    if (!modelsLoaded) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <p className="text-gray-500 mb-6">AI-powered face recognition attendance</p>

      {/* Config */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
        <div className="grid grid-cols-3 gap-4 mb-4">

          {/* ✅ Simple text input — no dropdown */}
          <div>
