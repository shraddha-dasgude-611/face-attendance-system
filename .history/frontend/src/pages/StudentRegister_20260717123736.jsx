...
export default function StudentRegister() {
  ...
  const startCamera = async () => {
    try {
      ...
      if (!modelsLoaded) {
        ...
        setCaptureMsg(loaded ? 'Ready! Position your face in frame' : '❌ Model loading failed');
      } else {
  ...
  const captureFace = async () => {
    ...
    try {
      ...
      if (!detection) {
        setCaptureMsg('❌ No face detected. Look directly at the camera.');
        return;
      }
      ...
      const angles = ['Look straight', 'Turn slightly left', 'Turn slightly right', 'Tilt up', 'Final capture!'];
      setCaptureMsg(`✅ Captured ${newCount}/${REQUIRED} — ${angles[newCount] || ''}`);
      if (newCount >= REQUIRED) setCaptureMsg('✅ All captures done! Ready to register.');
    } catch {
  ...
  if (step === 2) return (
    <div className="max-w-lg mx-auto p-6">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <p className="text-gray-500 mb-6">Take {REQUIRED} photos of <strong>{form.name}</strong></p>

        {/* Progress */}
        <div className="mb-6">
          <div className="flex justify-between text-sm text-gray-600 mb-2">
            <span>Progress</span>
            <span>{captureCount}/{REQUIRED}</span>
          </div>
        ...
        <div className="space-y-3">
          {!cameraActive ? (
            ...
            <button
              ...
              className="w-full bg-green-600 text-white py-4 rounded-lg font-semibold text-lg hover:bg-green-700 transition-colors disabled:opacity-50"
            >
              📸 Capture ({captureCount}/{REQUIRED})
            </button>
          )}
          ...
          <button
            onClick={() => { stopCamera(); setStep(1); setCaptureCount(0); setCapturedDescriptors([]); }}
  ...
  return (
    <div className="max-w-md mx-auto p-6">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center">
        ...
        <p className="text-gray-500">{form.name} has been registered successfully.</p>
        <p className="text-sm text-gray-400 mt-1">{captureCount} face samples captured.</p>
        <button
          onClick={resetForm}
          className="mt-6 w-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors"
        >
